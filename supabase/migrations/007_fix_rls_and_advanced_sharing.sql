...
-- Drop ALL existing overloads of is_admin() to prevent ERROR 42725
-- (function is not unique). Prior migrations (002/000) define is_admin()
-- with zero parameters; this migration defines is_admin(UUID DEFAULT ...).
-- PostgreSQL cannot disambiguate two zero-arg-callable overloads, so we
-- must drop every overload before (re)creating the canonical SECURITY DEFINER one.
