-- ═══════════════════════════════════════════════════════════════════════════
-- HealthFlux — Master SQL Setup (v4)
-- ═══════════════════════════════════════════════════════════════════════════
-- 
-- ONE-SHOT setup script for Supabase. Run this entire file in SQL Editor.
-- Safe to re-run — every statement is idempotent (IF NOT EXISTS, DROP IF EXISTS,
-- ON CONFLICT, etc.). Running twice will not break anything.
-- 
-- WHAT THIS CREATES:
--   • 39 tables (profiles, vitals, medications, documents, ...)
--   • Auto-create profile on signup with Free tier defaults
--   • Strict RLS — users only see their own data + admin bypass
--   • Document sharing ACL with expiry/password/max-views
--   • ABHA (Ayushman Bharat) account linkage
--   • Ad campaigns + admin-managed ads
--   • Supabase Storage buckets: documents (private), avatars/ads (public)
--   • Helper functions: is_admin(), can_upload_document(), assign_plan(), etc.
-- 
-- AFTER RUNNING:
--   1. Verify your account exists in profiles table
--   2. Run: SELECT make_admin('your-email@gmail.com');
--   3. Run: SELECT assign_plan('your-email@gmail.com', 'pro', 12);
--   4. Re-deploy your app: bash deploy.sh
-- 
-- Free Tier Limits (default for every new user):
--   • 4 family profiles
--   • 10 document uploads
--   • 10 AI calls
-- ═══════════════════════════════════════════════════════════════════════════
...
-- =====================================================================
-- BEGIN: 009_v4_critical_patch.sql  ★ NEW ★
-- =====================================================================
-- Purpose: Fixes profile-creation 500 error + adds production features.
-- 
-- This block fixes:
--   1. Recursive RLS infinite-loop on profiles (stack depth 54001)
--   2. Adds SECURITY DEFINER helpers (is_admin, current_user_email) 
--      that bypass RLS to avoid recursion
--   3. Auto-create profile on signup with default Free tier credits
--      (10 AI calls, 4 family profiles, 10 documents)
--   4. Document ACL — sharing with expiry, password, max-views
--   5. ABHA (Ayushman Bharat) account linkage table
--   6. Ad campaigns + ads management with admin-only writes
--   7. Storage RLS — user-scoped paths for documents
--   8. Helper functions: can_upload_document, can_add_profile
--
-- Safe to re-run.
-- =====================================================================
...
-- ─────────────────────────────────────────────────────────────────────
-- PART A: Add user_id (uuid) column to profiles for auth.uid()-based RLS
-- ─────────────────────────────────────────────────────────────────────
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Backfill user_id from auth.users where missing
UPDATE profiles p
SET user_id = u.id
FROM auth.users u
...
-- ─────────────────────────────────────────────────────────────────────
-- PART B: SECURITY DEFINER functions (THE FIX for stack-depth error)
-- ─────────────────────────────────────────────────────────────────────
-- These bypass RLS so policies checking "am I admin?" don't recurse.

-- Use CREATE OR REPLACE to keep same signature (no args) as the one defined earlier
-- in migration 002. Enhanced body uses SECURITY DEFINER to bypass RLS (prevents recursion).
...
AS $$
  SELECT EXISTS (
    ...
    WHERE (user_id = auth.uid() OR created_by = (SELECT email FROM auth.users WHERE id = auth.uid()))
      AND role = 'admin' 
...
-- ─────────────────────────────────────────────────────────────────────
-- PART C: REPLACE recursive profile policies with non-recursive ones
-- ─────────────────────────────────────────────────────────────────────
DROP POLICY IF EXISTS "profiles_select_strict" ON profiles;
DROP POLICY IF EXISTS "profiles_insert_strict" ON profiles;
DROP POLICY IF EXISTS "profiles_update_strict" ON profiles;
...
-- ─────────────────────────────────────────────────────────────────────
-- PART D: Replace policies on ALL data tables with admin-bypass support
-- ─────────────────────────────────────────────────────────────────────
DO $$ 
DECLARE
  tbl TEXT;
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('DROP POLICY IF EXISTS "%s_insert_strict" ON %I', tbl, tbl);
    EXECUTE format('DROP POLICY IF EXISTS "%s_update_strict" ON %I', tbl, tbl);
    EXECUTE format('DROP POLICY IF EXISTS "%s_delete_strict" ON %I', tbl, tbl);
    EXECUTE format('DROP POLICY IF EXISTS "%s_select_v2" ON %I', tbl, tbl);
    EXECUTE format('DROP POLICY IF EXISTS "%s_insert_v2" ON %I', tbl, tbl);
    EXECUTE format('DROP POLICY IF EXISTS "%s_update_v2" ON %I', tbl, tbl);
    EXECUTE format('DROP POLICY IF EXISTS "%s_delete_v2" ON %I', tbl, tbl);

    EXECUTE format(
      'CREATE POLICY "%s_select_v2" ON %I FOR SELECT USING (
        created_by = current_user_email() OR is_admin()
      )', tbl, tbl);
    EXECUTE format(
      'CREATE POLICY "%s_insert_v2" ON %I FOR INSERT WITH CHECK (
        created_by = current_user_email() OR is_admin()
      )', tbl, tbl);
    EXECUTE format(
      'CREATE POLICY "%s_update_v2" ON %I FOR UPDATE USING (
        created_by = current_user_email() OR is_admin()
      )', tbl, tbl);
    EXECUTE format(
      'CREATE POLICY "%s_delete_v2" ON %I FOR DELETE USING (
        created_by = current_user_email() OR is_admin()
      )', tbl, tbl);
  END LOOP;
END $$;
...
-- ─────────────────────────────────────────────────────────────────────
-- PART E: Auto-create profile + Free tier defaults on signup
-- ─────────────────────────────────────────────────────────────────────
-- Free tier: 4 family profiles, 10 documents, 10 AI calls
CREATE OR REPLACE FUNCTION handle_new_user()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
-- ─────────────────────────────────────────────────────────────────────
-- PART F: Free tier package defaults
-- ─────────────────────────────────────────────────────────────────────
UPDATE subscription_packages
...
-- Backfill existing free users to new defaults
UPDATE profiles
...
-- ─────────────────────────────────────────────────────────────────────
-- PART G: Document ACL (advanced sharing)
-- ─────────────────────────────────────────────────────────────────────
...
-- Extend medical_documents select to include shared docs
...
-- ─────────────────────────────────────────────────────────────────────
-- PART H: Supabase Storage buckets + policies
-- ─────────────────────────────────────────────────────────────────────
INSERT INTO storage.buckets (id, name, public) 
...
-- Storage policies (gracefully skip if insufficient privilege)
DO $$
BEGIN
  -- Documents: owner-only access + shared via ACL
  BEGIN
    ...
    -- Avatars: anyone read, only owner upload
...
-- ─────────────────────────────────────────────────────────────────────
-- PART I: Ads management (admin-managed)
-- ─────────────────────────────────────────────────────────────────────
...
-- Ensure ads table exists (may be added by 001 or not)
CREATE TABLE IF NOT EXISTS ads (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  created_by TEXT,
  title TEXT NOT NULL,
...
ALTER TABLE ads 
  ...
  ADD COLUMN IF NOT EXISTS placement TEXT DEFAULT 'dashboard',
...
-- Add CHECK constraints (drop first to avoid conflict on re-run)
DO $$
BEGIN
  ALTER TABLE ads DROP CONSTRAINT IF EXISTS ads_ad_type_check;
  ALTER TABLE ads ADD CONSTRAINT ads_ad_type_check 
    CHECK (ad_type IN ('banner','card','interstitial','sidebar'));
  ALTER TABLE ads DROP CONSTRAINT IF EXISTS ads_placement_check;
  ALTER TABLE ads ADD CONSTRAINT ads_placement_check 
    CHECK (placement IN ('dashboard','health_hub','wellness','documents','sidebar','footer'));
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Ad constraints already exist or unable to add';
END $$;
...
CREATE POLICY "ad_campaigns_admin_all" ON ad_campaigns FOR ALL USING (is_admin());

DROP POLICY IF EXISTS "ad_campaigns_public_read" ON ad_campaigns;
CREATE POLICY "ad_campaigns_public_read" ON ad_campaigns FOR SELECT USING (is_active = TRUE);

-- ─────────────────────────────────────────────────────────────────────
-- PART J: ABHA (Ayushman Bharat Health Account) integration
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- PART K: Helper functions for app logic
-- ─────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION can_upload_document(check_user_email TEXT DEFAULT NULL)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  email TEXT;
BEGIN
  email := COALESCE(check_user_email, current_user_email());
  
  SELECT COUNT(*) INTO current_count 
  FROM medical_documents WHERE created_by = email;

  SELECT sp.max_documents INTO max_allowed
  FROM profiles p
  JOIN subscription_packages sp ON sp.slug = p.plan_type
  WHERE p.created_by = email AND p.relationship = 'self'
  LIMIT 1;
...
CREATE OR REPLACE FUNCTION can_add_profile(check_user_email TEXT DEFAULT NULL)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT COUNT(*) INTO current_count FROM profiles WHERE created_by = email;
  SELECT sp.max_profiles INTO max_allowed
  FROM profiles p JOIN subscription_packages sp ON sp.slug = p.plan_type
  WHERE p.created_by = email AND p.relationship = 'self' LIMIT 1;
...
-- AI quota check (decrements credits)
CREATE OR REPLACE FUNCTION consume_ai_credit(check_user_email TEXT DEFAULT NULL)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT credits_remaining, plan_type INTO remaining, plan
  FROM profiles WHERE created_by = email AND relationship = 'self' LIMIT 1;
  ...
  WHERE created_by = email AND relationship = 'self';
  RETURN TRUE;
END;
$$;

-- ─────────────────────────────────────────────────────────────────────
-- PART L: Admin convenience functions
-- ─────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION make_admin(user_email TEXT)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
  UPDATE profiles 
  SET role = 'admin', updated_date = NOW()
  WHERE created_by = user_email AND relationship = 'self';
END;
$$;
...
AS $$
DECLARE
  pkg subscription_packages%ROWTYPE;
BEGIN
  SELECT * INTO pkg FROM subscription_packages WHERE slug = new_plan LIMIT 1;
  IF pkg IS NULL THEN RAISE EXCEPTION 'Unknown plan: %', new_plan; END IF;
  ...
  SET 
    ...
    subscription_status = 'active',
    subscription_ends_at = NOW() + (duration_months || ' months')::INTERVAL,
    credits_remaining = COALESCE(pkg.ai_calls_per_month, 10),
    credits_total = COALESCE(pkg.ai_calls_per_month, 10),
    updated_date = NOW()
  WHERE created_by = user_email AND relationship = 'self';
END;
$$;
...
-- =====================================================================
-- END: 009_v4_critical_patch.sql
-- =====================================================================

-- ─────────────────────────────────────────────────────────────────────
-- FINAL: Sanity check (uncomment to verify after running)
-- ─────────────────────────────────────────────────────────────────────
-- SELECT 'Tables:', COUNT(*) FROM pg_tables WHERE schemaname = 'public';
-- SELECT 'Policies:', COUNT(*) FROM pg_policies WHERE schemaname = 'public';
-- SELECT 'is_admin works:', is_admin();
-- SELECT 'current_user_email works:', current_user_email();
-- SELECT slug, name, ai_calls_per_month, max_profiles, max_documents FROM subscription_packages ORDER BY price_monthly;
